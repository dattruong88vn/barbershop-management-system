import {
  REPORT_ITEM_TAB_COMBO,
  REPORT_ITEM_TAB_SERVICE,
  REPORT_USAGE_SORT_ASC,
  REPORT_USAGE_SORT_DESC,
  REVENUE_REPORT_ITEM_TYPE_COMBO,
  REVENUE_REPORT_ITEM_TYPE_SERVICE,
  REVENUE_REPORT_TAB_BRANCH,
  REVENUE_REPORT_TAB_ITEM,
  StaffReportStatus,
  USER_ROLE_BARBER,
  USER_ROLE_MANAGER,
  USER_ROLE_OWNER,
  USER_ROLE_RECEPTIONIST,
  USER_ROLE_SKINNER,
} from "@/constants/common";
import type { ReportPageRole } from "@/types";

export const reportTexts = {
  api: {
    errors: {
      forbidden: "Forbidden",
      invalidDateRange: "Khoảng ngày báo cáo không hợp lệ.",
      serverError: "Server error",
      unauthorized: "Unauthorized",
    },
  },
  dateRange: {
    fromDateLabel: "Từ ngày",
    label: "Chọn thời gian",
    toDateLabel: "Đến ngày",
  },
  manager: {
    description:
      "Report API chưa implement, màn hình này đang giữ placeholder theo spec.",
    empty: "Chưa có dữ liệu báo cáo quản lý.",
    filtersTitle: "Bộ lọc",
    mockNotice:
      "Dữ liệu báo cáo quản lý đang là placeholder cho tới khi Report API hoàn tất.",
    tableTitle: "Bảng dữ liệu",
    title: "Báo cáo",
  },
  management: {
    branches: {
      title: "Báo cáo chi nhánh",
    },
    combos: {
      title: "Báo cáo combo",
    },
    revenue: {
      title: "Báo cáo doanh thu",
    },
    services: {
      title: "Báo cáo dịch vụ",
    },
    staff: {
      title: "Báo cáo nhân viên",
    },
  },
  personal: {
    comboCountLabel: "Combo đã làm",
    description: "Theo dõi số lượng dịch vụ, combo và dịch vụ nổi bật của bạn.",
    empty: "Chưa có dữ liệu báo cáo cá nhân.",
    fullNameLabel: "Họ và tên",
    generalInfoTitle: "Thông tin chung",
    chooseSpecificMonth: "Chọn tháng",
    loading: "Đang tải báo cáo cá nhân",
    metricCustomersServed: "Khách đã phục vụ",
    metricCompletedVisits: "Visit hoàn thành",
    monthPickerLabel: "Chọn tháng báo cáo",
    periodOptions: {
      all: "Tất cả thời gian",
      currentMonth: "Tháng hiện tại",
      month: "Tháng",
      year: "Năm hiện tại",
    },
    periodTitle: "Kỳ báo cáo",
    roleLabel: "Role",
    serviceCountLabel: "Dịch vụ đã làm",
    title: "Báo cáo cá nhân",
    topCustomerTitle: "Khách phục vụ nhiều nhất",
    topItemCount: (count: number) => `${count} lượt`,
    topItemTitle: "Dịch vụ thực hiện nhiều nhất",
  },
  roles: {
    [USER_ROLE_BARBER]: "Barber",
    [USER_ROLE_MANAGER]: "Quản lý",
    [USER_ROLE_OWNER]: "Chủ tiệm",
    [USER_ROLE_RECEPTIONIST]: "Reception",
    [USER_ROLE_SKINNER]: "Skinner",
  } satisfies Record<ReportPageRole, string>,
  branchReport: {
    allStatuses: "Tất cả",
    applyFilters: "Áp dụng",
    columns: {
      action: "Chi tiết",
      branch: "Chi nhánh",
      customers: "Khách",
      revenue: "Doanh thu",
      services: "Số dịch vụ/combo",
      status: "Trạng thái",
      visits: "Lượt ghé",
    },
    detailColumns: {
      barber: "Barber",
      completedAt: "Hoàn thành",
      customer: "Khách",
      receptionist: "Reception",
      revenue: "Doanh thu",
      services: "Số dịch vụ/combo",
      skinner: "Skinner",
      visit: "Visit",
    },
    detailPageSummary: "Trang {page}/{totalPages} · {total} visit",
    empty: "Chưa có dữ liệu báo cáo chi nhánh.",
    filtersTitle: "Bộ lọc",
    loading: "Đang tải báo cáo chi nhánh",
    loadingDetails: "Đang tải chi tiết báo cáo chi nhánh",
    periodLabel: "Kỳ báo cáo",
    retryDetails: "Thử lại",
    statusLabel: "Trạng thái",
    statusLabels: {
      active: "Đang hoạt động",
      inactive: "Ngừng hoạt động",
    },
    tableTitle: "Bảng chi nhánh",
    viewDetails: "Xem",
  },
  revenueReport: {
    applyFilters: "Áp dụng",
    branchLabel: "Chi nhánh",
    columns: {
      action: "Chi tiết",
      branch: "Chi nhánh",
      customers: "Khách",
      item: "Tên",
      itemType: "Loại",
      revenue: "Doanh thu",
      revenueShare: "Tỷ trọng",
      services: "Số dịch vụ/combo",
      usage: "Số lượt",
      visits: "Lượt ghé",
    },
    detailColumns: {
      barber: "Barber",
      branch: "Chi nhánh",
      completedAt: "Hoàn thành",
      customer: "Khách",
      item: "Tên",
      receptionist: "Reception",
      revenue: "Doanh thu",
      services: "Số dịch vụ/combo",
      skinner: "Skinner",
      visit: "Visit",
    },
    detailPageSummary: "Trang {page}/{totalPages} · {total} dòng",
    empty: "Chưa có dữ liệu báo cáo doanh thu.",
    filtersTitle: "Bộ lọc",
    itemTypes: {
      [REVENUE_REPORT_ITEM_TYPE_COMBO]: "Combo",
      [REVENUE_REPORT_ITEM_TYPE_SERVICE]: "Dịch vụ",
    },
    loading: "Đang tải báo cáo doanh thu",
    loadingDetails: "Đang tải chi tiết báo cáo doanh thu",
    periodLabel: "Kỳ báo cáo",
    pieTitles: {
      combo: "Tỷ trọng doanh thu combo",
      itemType: "Tỷ trọng dịch vụ/combo",
      service: "Tỷ trọng doanh thu dịch vụ",
    },
    retryDetails: "Thử lại",
    tableTitles: {
      branch: "Bảng chi nhánh",
      item: "Bảng dịch vụ/combo",
    },
    tabs: {
      [REVENUE_REPORT_TAB_BRANCH]: "Theo chi nhánh",
      [REVENUE_REPORT_TAB_ITEM]: "Theo dịch vụ/combo",
    },
    totalLabels: {
      customers: "Khách",
      revenue: "Doanh thu",
      services: "Số dịch vụ/combo",
      visits: "Lượt ghé",
    },
    unknownItem: "Chưa xác định",
    viewDetails: "Xem",
  },
  serviceComboReport: {
    allBranches: "Tất cả",
    allRoles: "Tất cả",
    applyFilters: "Áp dụng",
    branchLabel: "Chi nhánh",
    columns: {
      action: "Chi tiết",
      branch: "Chi nhánh",
      customers: "Khách",
      item: "Tên",
      responsibleRole: "Phụ trách",
      usage: "Số lượt sử dụng",
    },
    detailColumns: {
      branch: "Chi nhánh",
      completedAt: "Hoàn thành",
      customer: "Khách",
      item: "Tên",
      responsibleRole: "Phụ trách",
      visit: "Visit",
    },
    detailPageSummary: "Trang {page}/{totalPages} · {total} dòng",
    empty: "Chưa có dữ liệu báo cáo.",
    filtersTitle: "Bộ lọc",
    loading: "Đang tải báo cáo dịch vụ/combo",
    loadingDetails: "Đang tải chi tiết báo cáo",
    periodLabel: "Kỳ báo cáo",
    retryDetails: "Thử lại",
    roleLabel: "Vai trò phụ trách",
    searchLabel: "Tìm kiếm",
    searchPlaceholder: "Tìm theo tên",
    shopScope: "Tất cả chi nhánh",
    sortLabel: "Sắp xếp",
    sortOptions: {
      [REPORT_USAGE_SORT_ASC]: "Ít sử dụng nhất",
      [REPORT_USAGE_SORT_DESC]: "Sử dụng nhiều nhất",
    },
    tableTitle: "Bảng dữ liệu",
    tabs: {
      [REPORT_ITEM_TAB_COMBO]: "Combo",
      [REPORT_ITEM_TAB_SERVICE]: "Dịch vụ",
    },
    unknownRole: "Chưa xác định",
    viewDetails: "Xem",
  },
  staffReport: {
    allBranches: "Tất cả chi nhánh",
    allRoles: "Tất cả vai trò",
    branchLabel: "Chi nhánh",
    columns: {
      action: "Chi tiết",
      branch: "Chi nhánh",
      customers: "Khách",
      role: "Vai trò",
      services: "Số dịch vụ/combo",
      staff: "Nhân viên",
      status: "Trạng thái",
      visits: "Visit",
    },
    detailColumns: {
      branch: "Chi nhánh",
      completedAt: "Hoàn thành",
      customer: "Khách",
      item: "Dịch vụ/combo",
      role: "Phụ trách",
      visit: "Visit",
    },
    empty: "Chưa có dữ liệu báo cáo nhân viên.",
    filtersTitle: "Bộ lọc",
    currentBranchLabel: "Thông tin chi nhánh",
    detailPageSummary: "Trang {page}/{totalPages} · {total} dòng",
    applyFilters: "Áp dụng",
    loading: "Đang tải báo cáo nhân viên",
    loadingDetails: "Đang tải chi tiết báo cáo nhân viên",
    retryDetails: "Thử lại",
    managerScopeNotice: "Dữ liệu đang được giới hạn theo chi nhánh đang làm việc.",
    periodLabel: "Kỳ báo cáo",
    receptionistVisitItem: "Visit tiếp nhận",
    roleLabel: "Vai trò",
    statusLabels: {
      [StaffReportStatus.Active]: "Đang làm việc",
      [StaffReportStatus.BranchSuspended]: "Chi nhánh đóng cửa",
      [StaffReportStatus.Inactive]: "Đã nghỉ việc",
      [StaffReportStatus.Unknown]: "Thiếu phân công",
    },
    tableTitle: "Bảng nhân viên",
    unknownItem: "Dịch vụ chưa xác định",
    unknownRole: "Chưa xác định",
    unknownStaff: "Chưa xác định",
    viewDetails: "Xem",
  },
};
