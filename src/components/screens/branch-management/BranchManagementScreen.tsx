...
const DEFAULT_FILTERS: BranchFiltersValue = { search: "", status: BRANCH_STATUS_ALL };
...
export function BranchManagementScreen() {
  ...
  const filteredBranches = useMemo(() => {
    ...
    return branches.filter((branch) => {
      ...
      const matchesSearch = search ? `${branch.name} ${branch.address}`.toLowerCase().includes(search) : true;
      const matchesStatus = appliedFilters.status === BRANCH_STATUS_ALL || status === appliedFilters.status;
  ...
  const hasFilters = appliedFilters.search.trim().length > 0 || appliedFilters.status !== BRANCH_STATUS_ALL;

  return (
    <div className="min-h-screen bg-gray-200 px-4 py-8 text-gray-1000 sm:px-6 lg:px-8">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-4">
        <header><PageTitle>{branchTexts.ownerBranches.title}</PageTitle></header>
        <BranchFilters filters={draftFilters} onApply={applyFilters} onChange={setDraftFilters} />
        <BranchTable
