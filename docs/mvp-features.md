...
**Tổng quan vs Báo cáo:**

- Tổng quan dành cho owner/manager là overview nhanh + action alerts: theo dõi tình hình vận hành trong kỳ đang xem, top performers/items và cảnh báo cần xử lý như haircut thiếu ảnh.
- Tổng quan manager không hiển thị description dưới page title; filter kỳ dùng 3 tabs `Tháng`, `Năm hiện tại`, `Tất cả thời gian`, mặc định tháng hiện tại và mở chọn tháng bằng badge `Chọn tháng`.
- Toàn bộ widgets/charts/alerts của Tổng quan nằm trong một global card chung với title là tháng được chọn, `Năm hiện tại`, hoặc `Tất cả thời gian`.
...
- Cả Tổng quan và Báo cáo quản lý đều phải dùng `allocated_price` và snapshot fields cho doanh thu, không dùng giá/tên service hoặc combo hiện tại.
