...
- **Revenue report:** `/reports/revenue` cho owner và manager, gồm 2 tab `Theo chi nhánh` và `Theo dịch vụ/combo`. Filter chỉ gồm `Chọn thời gian` và nút `Áp dụng`, không có filter chi nhánh. Owner tab chi nhánh hiển thị tất cả chi nhánh có hoạt động trong kỳ; manager vẫn thấy tab chi nhánh nhưng dữ liệu bị scope theo active branch. KPI tổng gồm doanh thu, lượt ghé completed, khách unique và `Số dịch vụ/combo`. Tab chi nhánh liệt kê chi nhánh, lượt ghé, khách, số dịch vụ/combo, doanh thu và chi tiết. Tab dịch vụ/combo có 3 biểu đồ pie: tỷ trọng doanh thu `Dịch vụ`/`Combo`, tỷ trọng từng combo, tỷ trọng từng dịch vụ; bảng bên dưới liệt kê tên item, loại, số lượt, khách, doanh thu, tỷ trọng và chi tiết. Dịch vụ trong revenue report chỉ tính dịch vụ lẻ (`combo_id IS NULL`), không tính service con trong combo. Combo tính 1 combo trong 1 visit là 1 lượt, doanh thu combo là tổng `allocated_price` của các dòng combo đó.
