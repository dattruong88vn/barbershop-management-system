...
- **Filter action:** trong các màn hình báo cáo owner/manager, nút `Áp dụng` canh lề phải của filter grid và giữ kích thước tự nhiên của global `Button`; không kéo dài full-width theo cột.
...
- **Revenue report:** `/reports/revenue` cho owner và manager, gồm 2 tab `Theo chi nhánh` và `Theo dịch vụ/combo`. Filter chỉ gồm `Chọn thời gian` và nút `Áp dụng`, không có filter chi nhánh. Owner tab chi nhánh hiển thị tất cả chi nhánh có hoạt động trong kỳ; manager vẫn thấy tab chi nhánh nhưng dữ liệu bị scope theo active branch. KPI tổng gồm doanh thu, lượt ghé completed, khách unique và `Số dịch vụ/combo`. Tab chi nhánh liệt kê chi nhánh, lượt ghé, khách, số dịch vụ/combo, doanh thu và chi tiết. Tab dịch vụ/combo có 3 biểu đồ pie đặc, không dùng donut/inner radius: tỷ trọng doanh thu `Dịch vụ`/`Combo`, tỷ trọng từng combo, tỷ trọng từng dịch vụ. Pie chart dùng palette token `--chart-1..8`, ưu tiên xanh lá cho segment đầu tiên; legend nằm dưới chart với label/value canh thẳng hàng. Bảng bên dưới liệt kê tên item, loại, số lượt, khách, doanh thu, tỷ trọng và chi tiết. Dịch vụ trong revenue report chỉ tính dịch vụ lẻ (`combo_id IS NULL`), không tính service con trong combo. Combo tính 1 combo trong 1 visit là 1 lượt, doanh thu combo là tổng `allocated_price` của các dòng combo đó.
- **Revenue detail:** nút `Chi tiết` mở popup phân trang qua `GET /api/reports/revenue/details`, chưa link sang visit detail. Detail tab chi nhánh phân trang theo visit; detail tab item phân trang theo dòng dịch vụ lẻ hoặc theo visit chứa combo để tránh duplicate service con. Popup hiển thị ngày hoàn thành, chi nhánh, khách, receptionist, barber, skinner, item hoặc số dịch vụ/combo và doanh thu. Client cache summary theo tab + kỳ báo cáo, cache detail theo tab, drilldown id, item type, kỳ báo cáo, page và page size.
- **Service/Combo report:** `/reports/services` là màn duy nhất cho báo cáo dịch vụ và combo, có 2 tab `Dịch vụ` và `Combo`; tab nào active thì API trả danh sách tương ứng. Không tạo route riêng `/reports/combos`. Owner và manager đều xem được; manager bị scope theo active branch và không có filter chi nhánh, owner có filter chi nhánh. Filter gồm `Chọn thời gian`, search tên, sort `Sử dụng nhiều nhất`/`Ít sử dụng nhất`, nút `Áp dụng`; tab Dịch vụ có thêm filter `Barber`/`Skinner`. Summary không hiển thị doanh thu, gồm tên, chi nhánh, vai trò phụ trách nếu là dịch vụ, số lượt sử dụng, số khách và chi tiết. Tab Dịch vụ chỉ tính dịch vụ lẻ được chọn trực tiếp; không tính service con trong combo. Tab Combo tính một combo trong một visit là 1 lượt.
- **Service/Combo detail:** nút `Chi tiết` mở popup phân trang, chưa link sang visit detail. Dịch vụ detail theo dòng `visit_services` dịch vụ lẻ; combo detail theo visit có combo đó để tránh duplicate service con. Client cache detail theo item id, tab, chi nhánh, role, search, sort, kỳ báo cáo, page và page size.
- **Branch report:** `/reports/branches` chỉ dành cho owner. Hiển thị tất cả chi nhánh có ít nhất một ngày hoạt động giao với khoảng `fromDate`/`toDate`, kể cả chi nhánh hiện đã inactive. Filter gồm `Chọn thời gian`, trạng thái chi nhánh (`Tất cả`, `Đang hoạt động`, `Ngừng hoạt động`) và nút `Áp dụng`; không có dropdown chọn chi nhánh. Danh sách gồm chi nhánh, trạng thái, số lượt ghé hoàn thành, khách unique, `Số dịch vụ/combo`, doanh thu và nút chi tiết. `Số lượt ghé` tính theo visit `completed`; `Số dịch vụ/combo` tính `1 service = 1 lượt`, `1 combo = 1 lượt` dù combo được phân bổ thành nhiều dòng; doanh thu dùng snapshot `visit_services.allocated_price`.
- **Branch detail:** nút `Chi tiết` mở popup phân trang theo visit qua `GET /api/reports/branches/details`; chưa mở sang trang visit detail. Popup hiển thị ngày hoàn thành, khách, receptionist, barber, skinner, số dịch vụ/combo và doanh thu. Client cache detail theo branch id, kỳ báo cáo, trạng thái, page và page size.
...
- **Backend:** Staff personal report, staff management report, revenue report, branch report và service/combo report đã có API.
