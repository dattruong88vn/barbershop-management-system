# Developer Docs

Developer handbook cho Barber Shop SaaS. Dùng thư mục này khi cần hiểu cách setup, code, review, hoặc thay đổi hệ thống.

## Start Here

- Setup môi trường: [setup.md](setup.md)
- Runbook triển khai môi trường mới: [new-environment-runbook.md](new-environment-runbook.md)
- Kiến trúc: [architecture.md](architecture.md)
- Coding rules: [coding-rules.md](coding-rules.md)
- Data fetching: [data-fetching.md](data-fetching.md)
- Git flow: [git-flow.md](git-flow.md)

## Core Topics

- Auth và roles: [auth-and-roles.md](auth-and-roles.md)
- Tenancy: [tenancy.md](tenancy.md)
- API conventions: [api-conventions.md](api-conventions.md)
- Prisma và database: [prisma-and-db.md](prisma-and-db.md)
- Shared data database and location sync: [shared-location-db.md](shared-location-db.md)
- Testing: [testing.md](testing.md)

## Product/UI Docs

- Screen spec: [../SCREENS.md](../SCREENS.md)
- Design system: [../ui/design-system.md](../ui/design-system.md)
- Navigation: [../ui/navigation.md](../ui/navigation.md)
- Data model: [../data-model.md](../data-model.md)

## Module Docs

- Branches: [modules/branches.md](modules/branches.md)
- Visits: [modules/visits.md](modules/visits.md)
- Reports: [modules/reports.md](modules/reports.md)

## Decisions

- Tenancy model: [decisions/ADR-0001-tenancy-model.md](decisions/ADR-0001-tenancy-model.md)
- Visit pricing snapshot: [decisions/ADR-0002-visit-pricing-snapshot.md](decisions/ADR-0002-visit-pricing-snapshot.md)
- Feedback notification flow: [decisions/ADR-0003-feedback-notification-flow.md](decisions/ADR-0003-feedback-notification-flow.md)
- Staff mobile workflow: [decisions/ADR-0004-staff-mobile-workflow.md](decisions/ADR-0004-staff-mobile-workflow.md)
