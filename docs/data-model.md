...
| `hometown_province_code` | string | Mã tỉnh/thành quê quán, nullable; tham chiếu logic tới Shared Data DB |
| `current_province_code` | string | Mã tỉnh/thành nơi ở hiện tại, nullable; tham chiếu logic tới Shared Data DB |
...
> Các mã tỉnh/thành và phường/xã không có foreign key vật lý vì dữ liệu nằm trong Shared Data DB. API phải validate code và quan hệ ward-province trước khi lưu. `hometown` và `current_address` được giữ tạm thời cho dữ liệu legacy, không tự suy đoán mã hành chính từ chuỗi cũ.
